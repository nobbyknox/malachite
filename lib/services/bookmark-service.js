...
function setThumbnail(id, thumb, next) {
    ...
    dataStore.update(models.BookmarkModel, { 'id': id }, {}, { 'thumb': (thumb ? thumb : 'blank.png') }, function(err) {
        ...
        dataStore.create(models.BookmarkMetaModel, { 'bookmarkId': id, 'attr': constants.BOOKMARK_ATTR_THUMB_PROCESSED, 'value': helper.currentDatestamp()}, function(err) {

            if (err) {
                next(err);
            } else {
                dataStore.create(models.BookmarkMetaModel, { 'bookmarkId': id, 'attr': constants.BOOKMARK_ATTR_THUMB_UPDATED, 'value': helper.currentDatestamp()}, function(err) {
                    if (err) {
                        next(err);
                    } else {
                        dataStore.create(models.BookmarkMetaModel, { 'bookmarkId': id, 'attr': constants.BOOKMARK_ATTR_THUMB_UNREACHABLE, 'value': (!thumb)}, function(err) {
                            next(err);
                        });
                    }
                });
