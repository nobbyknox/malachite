...
let notificationService = require('./services/notification-service.js');
...
let tokenPolice = function(req, res, next) {
    ...
    if ((safePaths.indexOf(req.url) > -1) || (req.url.substring(0, 6) === '/assets')) {  // TODO: Should it not be substring(0, 7)?
        next();
    } else {
...
app.delete('/groups/:id', function(req, res) {
    bookmarkService.deleteGroup(req.params.id, req.query.__userId, function(err) {
        if (err) {
            res.status(constants.HTTP_INVALID_ENTITY).send(err);
        } else {
            res.status(constants.HTTP_OKAY).end();
...
app.delete('/tags/:id', function(req, res) {
    bookmarkService.deleteTag(req.params.id, req.query.__userId, function(err) {
        if (err) {
            res.status(constants.HTTP_INVALID_ENTITY).send(err);
        } else {
            res.status(constants.HTTP_OKAY).end();
...
app.post('/feedback', function(req, res) {
    notificationService.feedback(req.body, function(err) {
        if (err) {
            res.status(constants.HTTP_INVALID_ENTITY).send(err);
        } else {
            res.status(constants.HTTP_OKAY).send('');
