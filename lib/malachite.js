...
var tokenPolice = function(req, res, next) {
    ...
    var safePaths = ['/config', '/authenticate', '/validatetoken', '/system/nothumbs', '/system/updatethumbs', '/files', '/thumbs'];
...
app.delete('/bookmarks/:id', function(req, res) {
    dataStore.remove(models.BookmarkModel, {
        'id': req.params.id,
        'userId': req.query.__userId
    }, {}, true, function(err) {
        if (err) {
...
app.get('/bookmarks/group/:id', function(req, res) {
    ...
    dataStore.getWithDependents(models.GroupModel, {
        'id': req.params.id,
        'userId': req.query.__userId
    }, ['bookmarks'], function(err, groups) {
        if (err) {
            res.status(constants.HTTP_INVALID_ENTITY).send(err);
        } else {
...
app.get('/bookmarks/tag/:id', function(req, res) {
    ...
    dataStore.raw(util.format('select * from tags where userId = %d and name = \'%s\'', req.query.__userId, req.params.id), function(err, data) {
